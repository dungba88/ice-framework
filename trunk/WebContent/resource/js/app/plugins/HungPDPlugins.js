...
//plugin nay la cho de dat them cau hoi:
HungPDArticleSearchMorePlugin = Class.extend({
	init: function()	{
		this.name = "HungPDArticleSearchMorePlugin";
	},
	
	showSearchMoreButton: function()	{
		if ($('#effective-area .search-more-placeholder').length > 0)
			$('#effective-area .search-more-placeholder').show();
	},
	
	hideSearchMoreButton: function()	{
		if ($('#effective-area .search-more-placeholder').length > 0)
			$('#effective-area .search-more-placeholder').hide();
	},
	...
	onHtmlUpdated: function()	{
		...
		$('.extension-point[extensionName="PostResultList"]').each(function(index, value) {
			...
			$(value).append(tmpl('HungPDArticleSearchMorePluginTemplate', {}));
