AnswerEditPlugin = Class.extend({
	...
	submitAnswer : function(eventData) {
		...
		var obj = this;
		this.onAjax('answer', 'add', {questionID : qid, content : content}, 'POST', {
	...
	onEditAnswerSubmit: function(eventData)	{
		...
		var sbj = SingletonFactory.getInstance(Subject);
		this.onAjax('answer', 'update', {aid: aid, content: content}, 'POST',	{
			'onSuccess' : function(ret) {
				sbj.notifyEvent("NotifyGlobal", "Sửa thành công");
	...
	onAnswerBuilt : function() {
		...
		if (loggedin != 0)	{
			...
			$('.extension-point[extensionName="AnswerExtension"]').each(function()	{
				...
				if (curUID == uid)	{
					$(this).find('.ans-control').append(tmpl('AnswerEditPlugin-Control', {uid: uid, aid: aid}));
