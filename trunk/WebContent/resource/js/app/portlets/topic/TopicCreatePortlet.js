TopicCreatePortlet = Class.extend({
	...
	getAjaxAllTopics: function(){
		...
		$.ajax({url:curUrl, async:true, data:{}, success: function(ret){
			ret = $.parseJSON(ret);
			ret = ret.result;
	...
	onNewTopicConfirmed: function(){
		...
		for(var key in this.topicAvaiContextCommit){
			...
			var curUrl = this.root + "catchword/add-new-topic";
			var sbj = SingletonFactory.getInstance(Subject);
			$.ajax({url:curUrl, data: curObj, success: function(ret){
				ret = $.parseJSON(ret);
				if(!isNaN(ret.result)){
					curPlt.done++;
					sbj.notifyEvent("NotifyGlobal","Thêm thành công");
					sbj.notifyEvent("CommitTopicSuccess", {'value': ret.result, 'txt':curObj.topicName});
				}
				//curPlt.buildLocalTopic({text: curObj.topicName, value: curObj.topicName+curObj.contextName});
			}, error: function(){
