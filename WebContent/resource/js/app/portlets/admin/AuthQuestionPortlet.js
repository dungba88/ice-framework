...
AuthQuestionPortlet = Class.extend({
	init : function()	{
		this.name = "AuthQuestionPortlet";
	...
	onDeleteAuthQuestion: function(eventData)	{
		var i = eventData.id;
		var obj = {}; 
		obj.msg = "Bạn có thực sự muốn xóa dòng có id="+i+" không?";
		obj.func = "generateEvent('ConfirmDeleteAuthQuestion',{'id':"+i+"});";
		var str = tmpl(this.name+"-MsgBox",obj);
		$.facebox(str);
	},
	
	onConfirmDeleteAuthQuestion: function(eventData)	{
		...
		this.onAjax('ajax-admin', 'delete-auth-question', {'id':eventData.id}, 'POST', 
	...
	onEditAuthQuestion: function(eventData)	{
		var id = eventData.id;
		var question = {};
	...
	onConfirmEditAuthQuestion: function(eventData)	{
	...
	drawScreen: function()	{
		...
		this.onAjax('ajax-admin', 'list-all-auth-question', {}, 'POST', 
				{'onSuccess': function(ret)	{
					var ret;
					var objToSend = {};
					var i;
	...
	run: function(){
		this.loadExpertises();
		this.getPortletPlaceholder().getCanvas().access().parent().css("width","900px");
	}
