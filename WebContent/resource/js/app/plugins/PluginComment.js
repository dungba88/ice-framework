	...
	onAnswerBuilt: function(evenData)	{
		...
		if (uid != 0)	{
			$('.extension-point[extensionName="AnswerExtension"]').each(function()	{
				...
				$(this).find('.ans-control').append(tmpl('CommentPlugin-Control', {uid: uid, aid: aid}));
		...
		this.onAjax('answer', 'get-all-comments', {aid:aid}, 'GET', {
			'onSuccess': function(ret){
				...
				for(var key in ret){
					...
					objParam.date = ret[key]["created_date"];
		...
		$('.extension-point[extensionName="AnswerExtension"]').each(function()	{
			...
			obj.onAjax('answer', 'has-sent-negative-comment', {id: aid}, 'GET', {
				'onSuccess': function(ret)	{
					...
					$(dom).find('.ans-control').prepend(tmpl("ImproveAnswerPluginTmpl", {aid: aid}));
				}
			});
		});
	}
